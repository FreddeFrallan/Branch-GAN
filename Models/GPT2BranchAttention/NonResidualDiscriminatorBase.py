from Models.GPT2BranchAttention import NonResidualAttention
...
class NonResidualGPT2Block(torch.nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = NonResidualAttention.GPT2NonResidualAttention(config, layer_idx=layer_idx)
    ...
    def forward(self, hidden_states, layer_past=None, attention_mask=None, head_mask=None, encoder_hidden_states=None,
                ...
                promptMask=None, adapterLayer=None,
                customCasualMask=None, dropoutRate=None):
        # print("Dropout Block", dropoutRate is not None)
        ...
        attn_outputs = self.attn(
            ...
            nonResidual=nonResidual, nonResidualTextOnly=nonResidualTextOnly, nonResdiaulPastOnly=nonResdiaulPastOnly,
        ...
        if (adapterLayer is not None):
            hidden_states, _, residual = adapterLayer.pre_forward(hidden_states, residual, None)
            hidden_states, _, up = adapterLayer(hidden_states, residual_input=residual)
        else:
...
class NonResidualGPT2Model(modeling_gpt2.GPT2PreTrainedModel):
    ...
    def forward(self, input_ids=None, past_key_values=None, attention_mask=None, token_type_ids=None, position_ids=None,
                ...
                promptPast=None, textualPast=None, promptMask=None, customCasualMask=None,
                pTuningVectors=None, adapterLayers=None, insertBlocks=None, suffixOverrideEmbeddings=None,
                suffixOverrideEmbeddingsFront=False,
                dropoutRate=None,
                ):
        # print("Dropout Transformer", dropoutRate is not None)
        ...
        if (promptPast is None and textualPast is None):
            ...
            nonResidual = False
            nonResidualTextOnly = False
            nonResdiaulPastOnly = False
        elif (promptPast is not None and textualPast is None):
            # raise ValueError("You need to have a textual if you're gonna give a prompt past")
            textualPast = tuple([None] * len(self.h))
            nonResidual = False
            nonResidualTextOnly = False
            nonResdiaulPastOnly = True
        elif (promptPast is None and textualPast is not None):
            promptPast = tuple([None] * len(self.h))
            nonResidualTextOnly = True
            nonResidual = False
            nonResdiaulPastOnly = False
        else:
            nonResdiaulPastOnly = False
        ...
        if (suffixOverrideEmbeddings is not None):
            suffixLen = suffixOverrideEmbeddings.shape[1]
            if (suffixOverrideEmbeddingsFront):
                inputs_embeds = torch.cat((suffixOverrideEmbeddings, inputs_embeds[:, suffixLen:]), dim=1)
            else:
                inputs_embeds = torch.cat((inputs_embeds[:, :-suffixLen], suffixOverrideEmbeddings), dim=1)
        ...
        hidden_states = torch.nn.functional.dropout(hidden_states,
                                                    dropoutRate if dropoutRate is not None else self.config.embd_pdrop)
        if (pTuningVectors is not None):
            hidden_states = torch.cat((pTuningVectors, hidden_states), dim=1)  # Concat on SeqLen
            input_shape = (hidden_states.shape[0], hidden_states.shape[1])
        if (adapterLayers is None):
            adapterLayers = {i: None for i in range(len(self.h))}
        if (insertBlocks is None):
            insertBlocks = {}
        ...
        for i, (block, layer_past, layerPrompt, layerTextual) in enumerate(
            ...
            if (i in insertBlocks):
                block = insertBlocks[i]
            ...
            if self.model_parallel:
                ...
                if customCasualMask is not None:
                    customCasualMask = customCasualMask.to(hidden_states.device)
            ...
            if self.gradient_checkpointing and self.training:
                ...
                outputs = torch.utils.checkpoint.checkpoint(create_custom_forward(block),
                                                            ...
                                                            output_attentions=output_attentions,
                                                            nonResidual=nonResidual,
                                                            nonResidualTextOnly=nonResidualTextOnly,
                                                            nonResdiaulPastOnly=nonResdiaulPastOnly,
                                                            promptPast=layerPrompt,
                                                            textualPast=layerTextual,
                                                            promptMask=promptMask,
                                                            adapterLayer=adapterLayers[i],
                                                            customCasualMask=customCasualMask,
                                                            dropoutRate=dropoutRate,
                                                            )
...
class NonResidualGPT2LMHeadModel(modeling_gpt2.GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.valueHead = torch.nn.Linear(config.n_embd, 1, bias=False)
    ...
    def forward(self, input_ids=None, past_key_values=None, attention_mask=None, token_type_ids=None, position_ids=None,
                head_mask=None, inputs_embeds=None, encoder_hidden_states=None, encoder_attention_mask=None,
                customCasualMask=None, suffixOverrideEmbeddings=None, insertBlocks=None,
                labels=None, use_cache=None, output_attentions=None, output_hidden_states=None, return_dict=None,
                promptPast=None, textualPast=None, promptMask=None, returnLmLogits=False, dropoutRate=None):
        r"""
        ...
        """
        # print("Dropout GPT", dropoutRate is not None)
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        ...
        transformer_outputs = self.transformer(input_ids, past_key_values=past_key_values,
                                               ...
                                               customCasualMask=customCasualMask, insertBlocks=insertBlocks,
                                               suffixOverrideEmbeddings=suffixOverrideEmbeddings,
                                               dropoutRate=dropoutRate,
                                               )
        ...
        lm_logits = self.lm_head(hidden_states) if returnLmLogits else self.valueHead(hidden_states)
        # predictions = torch.sigmoid(lm_logits)
        ...
        # print("Loss: ", loss, labels is not None)
        if not return_dict:
